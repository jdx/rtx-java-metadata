...
function download {
	...
	local url="https://github.com/ibmruntimes/semeru11-certified-binaries/releases/download/${tag_name}/${asset_name}"
	...
	then
		...
		then
			...
			# shellcheck disable=SC2016
			regex='s/^ibm-semeru-certified-(jre|jdk)_(x64|x86-32|s390x|ppc64|ppc64le|aarch64)_(aix|linux|mac|windows)_.+_openj9-.+\.(tar\.gz|zip|msi)$/IMAGE_TYPE="$1" ARCH="$2" OS="$3" EXT="$4"/g'
		fi
...
download_github_releases 'ibmruntimes' 'semeru11-certified-binaries' "${RELEASE_FILE}"
...
jq -s -S . "${METADATA_DIR}"/ibm-semeru-certified-*.json > "${METADATA_DIR}/all.json"
